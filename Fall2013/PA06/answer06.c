...
struct Image * loadImage(const char* filename)
{
	...
	else
	{
		...
		size_t headerRead = fread(&header, sizeof(struct ImageHeader), 1, fp);  //Reads image header

		if(headerRead != 1) //Checks if header is correct size
		{
			fclose(fp);
			return NULL;
		}

		//Checks to see if magic bits is incorrect, width is 0 or height is 0
		...
		commentcheck = fread(imageData->comment, sizeof(char) * header.comment_len, 1, fp);
		wlcheck = fread(imageData->data, sizeof(uint8_t) * header.width * header.height, 1, fp);

		if (wlcheck != 1 || commentcheck != 1)
		{
			fclose(fp);
			free(imageData->data);
			free(imageData->comment);
...
void linearNormalization(struct Image * image)
{
	struct ImageHeader header;
	int i;
	int arraysize = header.height * header.width;
