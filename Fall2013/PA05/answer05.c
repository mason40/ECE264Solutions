...
char * * readString(char * filename, int * numString)
{
	...
	fscanf(fptr, "%s", buf); //read a word, stop at space
	fgets(buf, MAXIMUM_LENGTH, fptr);
	...
	*numString = numLine + 1;
	...
	strArr = malloc(sizeof(char *) * (numLine + 1));
...
{
	qsort(arrString, numString, sizeof(char*), cmpstringp);
}
